...
onready var flash = $Pivot/blasterF/Flash
onready var Decal = preload("res://Weapons/Decal.tscn")
onready var anim_play = $Pivot/Camera/AnimationPlayer
onready var health_hud = get_node("/root/Multiplayer/Container/V1/HUD")
...
func damaged(d):
	...
	if health <= 0:
		var _scene = get_tree().change_scene("res://UI/SPWin.tscn")
